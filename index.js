...
(function () {
  ...
  const utils = {
       ...
       * @returns {Element}
       */
      createElement: ({type = 'div', constantClass, extraAttributesArray = [], innerText, className}) => {
  ...
  class InspectorElement {
    constructor({pointerToDomEl, parent, key, isRoot}) {
      ...
      this.inspectorEl = this.updateChildren() //start recursive operation of mapping the DOM
        .boxLayout({renderSubtree : true}); // create inspector element layout base of the DOM el and its children.
    }
    ...
    boxLayout({renderSubtree = true}) {
      ...
      this.elChildrenContainer = utils.createElement({
        type: 'div',
        constantClass: constants.STYLE_CLASS.BOX_CHILDREN()
      });

      if (renderSubtree) this.boxChildrenLayout({elContainer});
    ...
    boxChildrenLayout({elContainer}) {
      this.children.forEach(child => {
        this.elChildrenContainer.appendChild(child.inspectorEl);
      });
      elContainer.appendChild(this.elChildrenContainer);
      return this;
    }
    ...
    moveElements({from, el, to}) {
      ...
      from.inspectorEl.setAttribute('style', constants.STYLE_CLASS.BOX({type : from.pointerToDomEl.nodeName, hasChildren : from.children.length > 0}));
      ...
      switch (lastDraggableInspectorElementPosition) {
        ...
        case constants.DRAG_POSITION.INSIDE:
          ...
          to.elChildrenContainer.appendChild(el.inspectorEl);
          to.children.push(el);
          break;
        /**
        ...
        case constants.DRAG_POSITION.LEFT:
          to.children.forEach(child => {
            if (child.key === this.key) {
              ...
              to.elChildrenContainer.insertBefore(el.inspectorEl, child.inspectorEl);
            }
        ...
        case constants.DRAG_POSITION.RIGHT:
          to.children.forEach(child => {
            ...
            if (child.key === this.key) {
              ...
              to.elChildrenContainer.insertBefore(el.inspectorEl, child.inspectorEl.nextSibling);
            }
          });
          to.children = newChildrenList;
          break;
      ...
      this.inspectorEl.setAttribute('style', constants.STYLE_CLASS.BOX({type : this.pointerToDomEl.nodeName, hasChildren : this.children.length > 0}));
    }
    ...
    isAllowed() {
      ...
      if (!lastDraggableInspectorElement || this.key === lastDraggableInspectorElement.key ) {
        return false; // the draggable element is parent of the target or arrived to the root without encountering the draggable element
      }
  ...
  const buildBottomInspector = () => {
    const containerEl = utils.createElement({type: 'div', className: 'root'});
